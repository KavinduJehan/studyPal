  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #4F8EF7 0%, #6DD5FA 100%)' }}>
      <div style={{ background: 'white', borderRadius: 16, boxShadow: '0 8px 32px rgba(79,142,247,0.15)', padding: 40, width: 350, maxWidth: '90vw', textAlign: 'center' }}>
        ...
        <h2 style={{ fontWeight: 400, color: '#222', marginBottom: 24 }}>Sign in to your account</h2>
        <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <button type="submit" style={{ background: 'linear-gradient(90deg, #4F8EF7 0%, #6DD5FA 100%)', color: 'white', fontWeight: 600, fontSize: 18, border: 'none', borderRadius: 8, padding: '12px 0', cursor: 'pointer', marginTop: 8, boxShadow: '0 2px 8px rgba(79,142,247,0.10)' }}>Login</button>
          {error && <div style={{ color: '#e53e3e', fontWeight: 500, marginTop: 8 }}>{error}</div>}
        </form>
        <p style={{ marginTop: 24, color: '#555', fontSize: 15 }}>
          Don't have an account? <Link to="/register" style={{ color: '#4F8EF7', fontWeight: 500, textDecoration: 'underline' }}>Register</Link>
        </p>
      </div>
    </div>
  );
